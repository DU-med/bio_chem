...
# test
######
    def set_gbk_gene_id(self, gene_id):
        """
        geneのIDを設定する
        """
        self.gene_id = gene_id
        handle = Entrez.efetch(db="gene", id=self.gene_id, rettype="gb", retmode="text")
        for i in handle:
            if "NC_" in i:
                info = i
        ...
        # GBKファイルの取得
        with Entrez.efetch(db="nucleotide", id=gene_acc,  rettype="gb", retmode="text", seq_start=start, seq_stop=end) as handle:
    ...
    def exon_list(self):
        ...
        for feature in self.record.features:
            if feature.type == 'mRNA' and feature.qualifiers['transcript_id'][0] == self.mrna_id:
                exon_ls = [[int(part.start), int(part.end)] for part in feature.location.parts]
        if self.strand == "+":
            return exon_ls
        else:
            return [[len(self.gDNA_seq())-j,len(self.gDNA_seq())-i] for i, j in exon_ls]
        ...
        for feature in self.record.features:
            if feature.type == 'mRNA' and feature.qualifiers['transcript_id'][0] == self.mrna_id:
                ...
                for i in range(len(ls)-1):
                    intron_list.append([ls[i][1],ls[i+1][0]])
        return intron_list
